...
```{r}
# Extract residuals
residuals_brms <- residuals(model_brms, type = "pearson")

# Plot residuals
hist(residuals_brms, main = "Histogram of Residuals", xlab = "Residuals")
```
...
```{r}
model_lme4_refined <- glmer(r2 ~ Gender + btype * mode + situ + (1 | id), 
                            data = VerbAgg, family = binomial(link = "logit"),
...
Based on the diagnostic results, the refined model with the interaction term (`btype:mode`) does not improve significantly compared to the simpler model (`model_lme4`). Therefore, I would revert to the simpler model (`model_lme4`).

### 2. `glmmTMB`: Refit with Alternative Link Functions

```{r}
model_glmmTMB_probit <- glmmTMB(r2 ~ Gender + btype + mode + situ + (1 | id), 
...
```
The diagnostics for the `probit` model (`model_glmmTMB_probit`) do not show significant improvement over the simpler `logit` model (`model_lme4`). Thus, I will keep using the `logit` model (`model_lme4`).
